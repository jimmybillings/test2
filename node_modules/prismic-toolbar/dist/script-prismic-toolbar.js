(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("PrismicToolbar", [], factory);
	else if(typeof exports === 'object')
		exports["PrismicToolbar"] = factory();
	else
		root["PrismicToolbar"] = factory();
})(this, function() {
...
var PREVIEW_COOKIE_KEY = 'io.prismic.preview';
var EXPERIMENT_COOKIE_KEY = 'io.prismic.experiment';
...
exports.default = {
  PREVIEW_COOKIE_KEY: PREVIEW_COOKIE_KEY,
  EXPERIMENT_COOKIE_KEY: EXPERIMENT_COOKIE_KEY,
  getPreviewToken: function getPreviewToken() {
    return getItem(PREVIEW_COOKIE_KEY);
  },
  getExperimentToken: function getExperimentToken() {
    return getItem(EXPERIMENT_COOKIE_KEY);
  },
  removeExperimentToken: function removeExperimentToken() {
    removeItem(EXPERIMENT_COOKIE_KEY);
  },
  setExperimentToken: function setExperimentToken(expId, variation) {
    var token = [expId, variation].join(' ');
    var expire = 60 * 60 * 24 * 30;
...
var _editbtn = __webpack_require__(3);

var _editbtn2 = _interopRequireDefault(_editbtn);

var _experiments = __webpack_require__(4);
...
function setupToolbar(endpoint) {
  if (endpoint) {
    ...
    _editbtn2.default.setup(endpoint);
  }
}

exports.default = {
  setup: _utils2.default.debounce(setupToolbar, 500),

  startExperiment: function startExperiment(expId) {
    _experiments2.default.start(expId);
  }
};
...
var _coookies = __webpack_require__(0);

var _coookies2 = _interopRequireDefault(_coookies);
...
function setup(endpoint) {
  var previewToken = _coookies2.default.getPreviewToken();
  var experimentToken = _coookies2.default.getExperimentToken();
  var matches = endpoint.match(new RegExp('(https?://([^/]*))'));
  if (!matches) return;
  ...
  fetch(baseURL + '/app/authenticated/v2', {
    ...
    if (contentType && contentType.indexOf('application/json') !== -1) {
      response.json().then(function (json) {
        if (json.userId) {
          document.querySelectorAll('.wio-link').forEach(function (el) {
            el.parentNode.removeChild(el);
          });

          document.querySelectorAll('[data-wio-id]').forEach(function (el) {
            var documentId = el.dataset.wioId;
            var url = function () {
              if (previewToken) {
...
function loadGoogleExperimentScript(googleId, callback) {
  var src = '//www.google-analytics.com/cx/api.js?experiment=' + googleId;
  var alreadyExists = document.querySelector('[src="' + src + '"]');
...
function start(googleId) {
  loadGoogleExperimentScript(googleId, function () {
    try {
      if (!_coookies2.default.supportCookies()) return;
      var previewToken = _coookies2.default.getPreviewToken();
      if (!previewToken) {
        var inCookie = (_coookies2.default.getExperimentToken() || '').split(' ');
        var googleVariation = window.cxApi.chooseVariation();
        if (googleVariation === window.cxApi.NOT_PARTICIPATING) {
          _coookies2.default.removeExperimentToken();
          return;
        }
        if (inCookie[0] !== googleId || inCookie[1] !== googleVariation.toString()) {
          _coookies2.default.setExperimentToken(googleId, googleVariation);
...
function closeSession() {
  ...
  var domain = void 0;var path = void 0;
  var sizeWithoutLastPathPart = void 0;
  domainParts.forEach(function (domainPart, domainPartIndex) {
    domain = domainParts.slice(domainPartIndex).join('.');
    pathParts.forEach(function (pathPart, pathPartIndex) {
      sizeWithoutLastPathPart = pathParts.length - 1;
      path = pathParts.slice(pathPartIndex, sizeWithoutLastPathPart).join('/');
      _coookies2.default.removeItem(_coookies2.default.PREVIEW_COOKIE_KEY, path + '/', domain);
      _coookies2.default.removeItem(_coookies2.default.PREVIEW_COOKIE_KEY, path + '/', '.' + domain);
    });
  });
}
...
function setup() {
  try {
    ...
    var existingToolbar = document.querySelector('#io-prismic-toolbar');
...
document.addEventListener('DOMContentLoaded', function () {
  if (window.prismic && window.prismic.endpoint) {
    _index2.default.setup(window.prismic.endpoint);
  }
});

exports.setup = _index2.default.setup;
exports.startExperiment = _index2.default.startExperiment;
...
//# sourceMappingURL=script-prismic-toolbar.js.map